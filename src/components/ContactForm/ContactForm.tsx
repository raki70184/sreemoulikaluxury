...
import { Alert, Button, Grid, Snackbar, TextField, Typography, useMediaQuery, useTheme } from "@mui/material";
...
const ContactForm: React.FC = () => {
  ...
  return (<>
    {submitted ? <Grid container alignItems={"center"} textAlign={"center"} justifyContent={"center"} spacing={3} marginTop={2}>
      <Grid item>
        <Alert severity="success" sx={{fontStyle: "italic"}}>
          The information has been submitted successfully !
        </Alert>
      </Grid>
      <Grid item sx={{fontStyle: "italic"}}>DM <a target="blank" className="instagram" href="https://instagram.com/sm.luxe.hyderabad?igshid=OGQ5ZDc2ODk2ZA==">@Smluxehyderabad</a> for any other queries !!</Grid>
    </Grid> :
      <Grid container flexDirection={"row"} padding={2} textAlign={sm ? "center" : "left"}>
        ...
        <form id="contact-form" style={{ width: "100%" }} ref={form} action={PostDetailsUrl} method="POST" onSubmit={submitHanlder}>
          <Grid item container flexDirection={"column"} marginTop={2} className="contact-form" paddingRight={sm ? 0 : 2}>
            ...
            <Grid item marginTop={2}>
              <Button type="submit" color="warning" variant="contained" size="medium" sx={{borderRadius: "10px"}} fullWidth={sm} >
